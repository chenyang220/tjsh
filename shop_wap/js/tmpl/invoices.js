    ...
    function goodsDetail(){
        ...
        $.ajax({
            ...
            success:function(result){
                ...
                if(result.data.IsOpenInv==0)
                {
                    $('#invoice-valve').parent().parent().hide();
                }
...
function add_invoice(invoice_type){
    ...
    if(invoice_type == 'electronics'){
        ...
        var title = obj.find('.checked').find("input[name='inv_ele_title_type']").val() == 'company'  ?$('.company_tit').val() : obj.find('.checked').find("input[name='inv_ele_title']").val();

        var invContent = '电子发票'+' '+obj.find('.checked').find("input[name='inv_ele_title']").val()+' '+ cont;
        var invoice_code = obj.find("input[name='company_tax_num']").val();
        $('.order_invoice_fee1').css('display','none');
    ...
    if(invoice_type == 'increment'){
        ...
        if(a==1&&result.status == 200){
            $('#order_invoice_fee').html('￥'+result.data.order_invoice_fee);
            $('#totalPrice').html($('#totalPrice').html()*1+(result.data.order_invoice_fee*1));
            $('.order_invoice_fee1').css('display','block');
            a=2;
        }
    }else{
        var data = {
