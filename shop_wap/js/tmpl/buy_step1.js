...
$.ajax({
    type: 'post',
    url: ApiUrl + "/index.php?ctl=Buyer_Cart&met=getValue&typ=json",
    ...
    * */
    ...
    var insertHtmlAddress = function (address, address_id) {
        ...
        for (var i = 0; i < address.length; i++) {
            if (address_id != 0) {
                if (address[i].user_address_id == address_id) {
                    ...
                    address_info.user_address_code = address[i].user_address_code;
                }
            } else {
        ...
        if (!isEmptyObject(address_info)) {
            ...
            $('#mob_phone').html(address_info.user_address_phone);
            $('#address').html(address_info.user_address_area + address_info.user_address_address);
            $('#user_address_code').html(address_info.user_address_code);
        } else {
    ...
    $('#add_address_form').find('.btn').click(function () {
        if ($.sValid()) {
            ...
            param.user_address_code = $('#address_code').val();
    ...
    $('#ToBuyStep2').click(function () {
        ...
        if($('#invoice-valve').parent().parent().css('display') != 'none')
        {
            if(invoice_id==0)
            {
                $.sDialog({
