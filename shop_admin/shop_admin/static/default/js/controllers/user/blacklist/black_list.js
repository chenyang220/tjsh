...
var THISPAGE = {
    ...
    loadGrid: function(){
        ...
        $("#grid").jqGrid({
            url:SITE_URL +  '?ctl=User_BlackList&met=getInfoList&typ=json',
        ...
        function operFmatter (val, opt, row) {
            var html_con = '<div class="operating" data-id="' + row.id+ '"><span  title="恢复" style ="border:1px solid grey;border-radius: 5px;display:block;" id="add-black-list">恢复</span></div>';
            return html_con;
        };
...
function addblack(e){
    parent.$.dialog.confirm('确认要恢复该用户的购买权限吗？', function () {
        Public.ajaxPost(SITE_URL + '?ctl=User_BlackList&met=editInfo&typ=json', {user_id:e}, function (data) {
            if (data.status == 200) {
