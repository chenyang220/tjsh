...
var THISPAGE = {
    ...
    initDom: function(){
        ...
        $('#send-fromDate').datetimepicker({lang:'ch'}).prop('readonly', 'readnoly');
        $('#send-toDate').datetimepicker({lang:'ch'}).prop('readonly', 'readnoly');
        $('#draw-fromDate').datetimepicker({lang:'ch'}).prop('readonly', 'readnoly');
        $('#draw-toDate').datetimepicker({lang:'ch'}).prop('readonly', 'readnoly');
    },
    loadGrid: function(){
        ...
        var colModel = [
           ...
           {name:'member_id', label:'用户编号', width:200, align:"center"},
           {name:'tel_phone', label:'手机号', width:200, align:"center"},
            {name:'game_code', label:'抽奖码序号', width:200, align:"center"},
            {name:'send_time', label:'抽奖码发送时间', width:200, align:"center"},
            {name:'is_used_text', label:'使用情况', width:200, align:"center"},
            {name:'game_id', label:'抽奖活动id',  width:150, align:"center"},
            {name:'game_date', label:'抽奖时间',  width:150, align:"center"},
            {name:'game_goods', label:'奖品',  width:150, align:"center"},
        ];
        this.mod_PageConfig.gridReg('grid', colModel);
        colModel = this.mod_PageConfig.conf.grids['grid'].colModel;
        $("#grid").jqGrid({
            url: SITE_URL + '?ctl=User_Game&met=DrawCodeManage&typ=json',
    ...
    addEvent: function(){
        ...
        $('#search').click(function(){
            ...
            if ( $('#send-fromDate').val() ) {
                queryConditions.send_date_f = $('#send-fromDate').val();
            }
            if ( $('#send-toDate').val() ) {
                queryConditions.send_date_t = $('#send-toDate').val();
            }

            if ( $('#draw-fromDate').val() ) {
                queryConditions.draw_date_f = $('#draw-fromDate').val();
            }
            if ( $('#draw-toDate').val() ) {
                queryConditions.draw_date_t = $('#draw-toDate').val();
            }
