...
var THISPAGE = {
    ...
    loadGrid: function(){
        ...
        var colModel = [
            ...
            {name:'user_pv', label:'用户PV值', width:100, align:"center"},
            {name:'user_birthday', label:'出生日期', width:150, align:"center"},
            {name:'user_regtime', label:'注册时间', width:150, align:"center"},
            {name:'shop_type', label:'商家 类型', width:100, align:"center","formatter":shop_type},
        ...
        function operFmatter (val, opt, row) {
            var html_con = '<div class="operating" data-id="' + row.id+ '"><span  title="加入黑名单" style ="border:1px solid grey;border-radius: 5px;" id="add-black-list">加入黑名单</span></div>';
            return html_con;
        };
    ...
    addEvent: function(){
        ...
        //编辑
        $('.grid-wrap').on('click', '#add-black-list', function(e){
            e.preventDefault();
...
function addblack(e){
    parent.$.dialog.confirm('加入黑名单，该用户将不享有购买权限', function () {
        Public.ajaxPost(SITE_URL + '?ctl=User_Info&met=editInfo&typ=json', {user_id:e}, function (data) {
            if (data.status == 200) {
