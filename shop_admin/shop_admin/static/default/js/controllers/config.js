...
$(function ()
{
    if ($('#order-setting-form').length > 0)
    {
        $('#order-setting-form').validator({
            ignore: ':hidden',
            theme: 'yellow_bottom',
            timely: 1,
            stopOnError: true,
            ...
            valid: function (form)
            {
                parent.$.dialog.confirm('修改立马生效,是否继续？', function ()
                    {
                        Public.ajaxPost(SITE_URL + '?ctl=Config&met=edit&typ=json', $('#order-setting-form').serialize(), function (data)
                        {
                            if (data.status == 200)
                            {
                                parent.Public.tips({content: '修改操作成功！'});
...
$(function ()
{
    if ($('#index_dajiashuo-setting-form').length > 0)
    {
        $('#index_dajiashuo-setting-form').validator({
            ignore: ':hidden',
            theme: 'yellow_bottom',
            timely: 1,
            stopOnError: true,
            ...
            valid: function (form)
            {
                parent.$.dialog.confirm('修改立马生效,是否继续？', function ()
                    {
                        Public.ajaxPost(SITE_URL + '?ctl=Config&met=edit&typ=json', $('#index_dajiashuo-setting-form').serialize(), function (data)
                        {
                            if (data.status == 200)
                            {
                                parent.Public.tips({content: '修改操作成功！'});
    ...
    if ($('#operation-setting-form').length > 0)
    {
        ...
        $('#operation-setting-form').validator({
            ...
            valid: function(form){
                ...
                if(!pv || !/^[1-9][0-9]{0,6}$|^[1-9][0-9]{0,6}\.[0-9]{0,1}[1-9]$|^0$|^[0-9]\.[0-9]{0,1}[1-9]$/.test(pv)){
                    parent.Public.tips({type: 1, content: 'PV值必须为0.00-999999.99的数字'});
                    return false;
                }
                parent.$.dialog.confirm('修改立马生效,是否继续？', function () {
                    Public.ajaxPost(SITE_URL + '?ctl=Config&met=edit&typ=json', $('#operation-setting-form').serialize(), function (data) {
