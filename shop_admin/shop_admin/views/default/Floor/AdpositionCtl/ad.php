...
//var_dump($data);exit;
    ...
    <div class="tips"><i class="iconfont"></i>请上传宽度<?php if($data['layout_id']==4){?>
            <?php echo 960?> 
        <?php }else{ ?>
            <?php echo ceil($data['width'])?>
        <?php }?>
     像素、高度
      <?php if($data['layout_id']==4){?>
             <?php echo 802?>
        <?php }else{ ?>
            <?php echo ceil($data['height'])?>
        <?php }?>像素的图片，过大或过小的图像都将影响显示效果正常</div>
    ...
    <input type="hidden" name="widget_id" value="<?php echo $data['adimgtext']['widget_id'] ?>" id="widget_id"/>
     ...
     <div class="tips"><i class="iconfont">&#xe65e;</i>请上传宽度
        <?php if($data['layout_id']==4){?>
            <?php echo 960?>
        <?php }else{ ?>
            <?php echo ceil($data['width'])?>
...
<script type="text/javascript" src="<?=$this->view->js?>/controllers/config.js" charset="utf-8"></script>
    ...
    <script>
        ...
        setting_logo_upload = new UploadImage({
            thumbnailWidth: <?php if($data['layout_id']==4){echo 960;}else{echo substr($data["width"],0,strlen($data["width"])-2);} ?>,
            thumbnailHeight:<?php if($data['layout_id']==4){echo 802;}else{echo substr($data["height"],0,strlen($data["height"])-2);}?>,
