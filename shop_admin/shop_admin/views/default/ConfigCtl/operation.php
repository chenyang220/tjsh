...
<div class="wrapper page">
    ...
    <form method="post" id="operation-setting-form" name="settingForm">
        ...
        <div class="ncap-form-default">
            ...
            <dl class="row">
                <dt class="tit">会员折扣功能</dt>
                <dd class="opt">
                    <ul class="nofloat">
                        <li>
                            <div class="onoff" style="height: 24px;">
                                <label title="开启" class="cb-enable <?=($data['rate_service1_status']['config_value']==1 ? 'selected' : '')?> " for="rate_service1_enable">开启</label>
                                <label title="关闭" class="cb-disable <?=($data['rate_service1_status']['config_value']==0 ? 'selected' : '')?>" for="rate_service1_disabled">关闭</label>
                                <input type="radio" value="1" name="operation[rate_service1_status]" id="rate_service1_enable" <?=($data['rate_service1_status']['config_value']==1 ? 'checked' : '')?> />
                                <input type="radio" value="0" name="operation[rate_service1_status]" id="rate_service1_disabled" <?=($data['rate_service1_status']['config_value']==0 ? 'checked' : '')?> />
                            </div>
                        </li>
                    </ul>
                    <p class="notic">开启状态下，可以享有会员折扣功能，关闭状态，会员折扣功能关闭</p>
                </dd>
            </dl>
            ...
            <dl class="row">
                <dt class="tit">商品评价功能</dt>
                <dd class="opt">
                    <ul class="nofloat">
                        <li>
                            <div class="onoff" style="height: 24px;">
                                <label title="开启" class="cb-enable <?=($data['goods_evaluate']['config_value']==1 ? 'selected' : '')?> " for="goods_evaluate_enable">开启</label>
                                <label title="关闭" class="cb-disable <?=($data['goods_evaluate']['config_value']==0 ? 'selected' : '')?>" for="goods_evaluate_disabled">关闭</label>
                                <input type="radio" value="1" name="operation[goods_evaluate]" id="goods_evaluate_enable" <?=($data['goods_evaluate']['config_value']==1 ? 'checked' : '')?> />
                                <input type="radio" value="0" name="operation[goods_evaluate]" id="goods_evaluate_disabled" <?=($data['goods_evaluate']['config_value']==0 ? 'checked' : '')?> />
                            </div>
                        </li>
                    </ul>
                    <p class="notic">开启状态下商品可以进行评价，关闭状态，所有评价都将关闭</p>
                </dd>
            </dl>
            ...
            <dl class="row">
                <dt class="tit">
                    <label for="invoice_price">专票邮费设置</label>
                </dt>
                <dd class="opt">
                    <input id="invoice_price" name="operation[invoice_price]" value="<?=($data['invoice_price']['config_value'])?>" class="ui-input w346" type="text" onkeyup="this.value=this.value.replace(/[^\d].[^\d]/g,'') " onafterpaste="this.value=this.value.replace(/[^\d].[^\d]/g,'')"/><i>元</i>

                    <p class="notic">输入0-999.9之内的数字，用户需开专业发票，会收取对应的邮费</p>
                </dd>
            </dl>
