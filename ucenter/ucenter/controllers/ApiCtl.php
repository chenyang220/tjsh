...
//        }
	...
	*/
	...
	//计划任务查询会员数据异动纪录需要查询本商城会员
	public function usersInfo()
	{
		$User_InfoModel = new User_InfoModel();
		$User_InfoDetailModel = new User_InfoDetailModel();
		$cond_row['user_name:IN'] = request_string('member_id');
		$result = $User_InfoDetailModel->getInfoDetailList($cond_row);
		...
		$data = array();
		$this->data->addBody(100, $result,$msg, $status);
	}
	
	/** 
     * @会员状态变更,修改B2C会员订购状态
     * @MemberID,MemberStatus,OrderFlag
     * @key
   ...
   public function UpMebStatus(){
       ...
       $key = 'CmqD6kpCBq7b7ypB';
        ...
        // $result['MemberID']      = "5133708";  // 
        // $result['MemberStatus']  = "N";  //   N
        // $result['OrderFlag']     = "Y";//  Y

        // AES加密
        $aes        = new AesCrypt();
        $aes->iv    = $key;  //替换iv
       
        $result = $aes->decrypt($_REQUEST['data'],$key); //解密,无需转码,返回json数据
        ...
        $userinfo1= $User_InfoModel->getOneByWhere(array('user_name'=>$userinfo['user_name'])); //拿出user_id用于shop端修改订购状态
        ...
        $data  = $aes->encrypt(json_encode($data,320),$key); //AES加密返回 320防止中文转译乱码或日期多反斜杠
        //$data  = '='.urlencode($data); //转码并在前面连接"="
		header('Content-type: application/json');
		echo $data;
        // echo  json_encode(array('=9KFeC1EHruByRc%2B7JKHDRsrgHL3Ga28cXBpwsxMVsQV17PLgMis5%2B1EtxCPLlKnxaIMxgJGgktdkKOZ2rYa35A%3D%3D'));
        // return $this->data->addBody(-100,array($data),$msg,$status);
    }
    ...
    public function editUserLogo(){
        ...
        $user_avatar = request_string('user_logo');

        $user_id = request_int('user_id');

        $cond_row = array('user_avatar'=>$user_avatar);
        ...
        $flag = $User_InfoDetailModel->editInfoDetail($user_row['user_name'],$cond_row);
