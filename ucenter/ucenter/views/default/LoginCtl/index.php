...
<head>
    ...
    <link rel="stylesheet" type="text/css" href="<?= $this->view->css ?>/login_b.css" />
...
<!--				<img src="--><?//= $web['site_logo'] ?><!--" height="60"/>-->
                <img class="logoIcon" src="<?= $web['site_logo'] ?>" height="60"/>
            </a>
            <b>欢迎登录</b>
        </div>
	...
	<div id="content">
		<div class="login-cont" style="background:<?=Web_ConfigModel::value('login_backcolor')?>">
            <img class="bdImg" src="ucenter\static\default\images\login_01\bd_02.jpg" alt="">
			<div class="login-wrap login-wrap-content" style="background: url(<?=Web_ConfigModel::value('login_logo')?>) no-repeat -1px;">
				<div class="login-form">
					...
					<a href="javascript:history.go(-1)" class="back-pre"></a>
                        ...
                        <!-- <a href="<?=sprintf('%s?ctl=Login&act=reg&t=%s&from=%s&callback=%s', Yf_Registry::get('url'), request_string('t'), request_string('from'), urlencode(request_string('callback')))?>" class="back-to-regist" target="_blank">注册 》</a> -->
                        <a href="<?=sprintf('%s?ctl=Login&act=regidimg&t=%s&from=%s&callback=%s', Yf_Registry::get('url'), request_string('t'), request_string('from'), urlencode(request_string('callback')))?>" class="back-to-regist" target="_blank">注册 》</a>
					</div>
					<div class="login-box" style="visibility: visible;">
<!--                        icon-->
                        
                        <div class="portraitIcon">
                            <img style="height: auto;" src="<?= $web['site_logo'] ?>" alt="">
                        </div>
						<div class="mt tab-h" style="display:none;">
						</div>
						<!-- <div class="msg-wrap" style="display:none;">
							<div class="msg-warn"><b></b>公共场所不建议自动登录，以防账号丢失</div>
							<div class="msg-error register-error"></div>
						</div> -->
						<div class="mc">
							<div class="form">
								<form id="formlogin" method="post" onsubmit="return false;">
									...
									<div class="item item-fore1">
										...
										<input id="loginname" type="text" class="itxt lo_user_account" name="user_account" tabindex="1" autocomplete="off" placeholder="会员编号/手机">
...
<!--                                            </span>-->
											<span class="forget-pw-safe">
												<a href="<?=sprintf('%s?ctl=Login&act=findmebid&t=%s&from=%s&callback=%s', Yf_Registry::get('url'), request_string('t'), request_string('from'), urlencode(request_string('callback')))?>" class="" target="_blank" >忘记会员编号</a>
                                    ...
                                    <div class="wap-show">
<!--                                        <a href="--><?//=sprintf('%s?ctl=Login&act=reg&t=%s&from=%s&callback=%s', Yf_Registry::get('url'), request_string('t'), request_string('from'), urlencode(request_string('callback')))?><!--" target="_blank"><b></b>立即注册</a>|-->
                                        <a href="<?=sprintf('%s?ctl=Login&act=findmebid&t=%s&from=%s&callback=%s', Yf_Registry::get('url'), request_string('t'), request_string('from'), urlencode(request_string('callback')))?>" class="" target="_blank" >忘记会员编号</a> |
                                        <a href="<?=sprintf('%s?ctl=Login&act=reset&t=%s&from=%s&callback=%s', Yf_Registry::get('url'), request_string('t'), request_string('from'), urlencode(request_string('callback')))?>" class="" target="_blank" >忘记密码</a>
                                    </div>
									...
									<div class="item item-fore5">
										...
										<div class="register-btn ">
											<a href="<?=sprintf('%s?ctl=Login&act=regidimg&t=%s&from=%s&callback=%s', Yf_Registry::get('url'), request_string('t'), request_string('from'), urlencode(request_string('callback')))?>" class="btn-img btn-entry" id="" tabindex="6">注&nbsp;&nbsp;&nbsp;&nbsp;册</a>
										</div>
									</div>
...
<!--    弹窗层-->
    <div class="timeSelectBox">
        <div class="TeStSon">
            <p class="TeStSon_p">请完成首次注册,即可进入商城</p>
            <button class="TeStSon_bn">确定</button>
        </div>
...
<script>
    var w = $(window).width();
    if( w > 768){
        $("#loginsubmit").html("立即登录");
        // $(".TeStSon_p").html("请完成首次注册即可进入商城");
        $(".TeStSon_bn").html("确定");
    }
    function PopUp(r,t){
        console.log("002")
