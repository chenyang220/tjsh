...
<!--		</a>-->
		...
		<div class="logo-title">忘 记 密 码</div>
...
<div class="container w">
	<div id="header"><a href="javascript:history.go(-1)" class="back-pre"></a>忘记密码</div>
    <div class="portraitIcon tc">
        <img src="<?= $web['site_logo'] ?>" alt="">
    </div>
	<div class="main clearfix" id="form-main">
		<div class="reg-form forget_form fl">
			<form action="" id="register-form" class="forget_Form" method="post" novalidate="novalidate" onsubmit="return false;">
...
<!--                        <label>手机验证码：</label>-->
                        ...
                        <input type="text" name="mobileCode" maxlength="6" id="phoneCode" class="field phonecode  re_mobile forget_me" placeholder="请输入手机验证码" autocomplete="off">
...
<!--					<label>重 置 密 码：</label>-->
					...
					<input type="password" id="re_user_password" class="field re_user_password" autocomplete="off" maxlength="20" placeholder="请输入新密码建议至少使用两种字符组合" default="<i class=i-def></i><?=$pwd_str?>" onfocus="checkPwd()" onblur="pwdCallback()">
...
<!--					<label>确 认 密 码：</label>-->
				...
				<div>
					<button type="submit" class="btn-register forget-register" onclick="resetPasswdClick()">完成</button>
				</div>
...
<script>
	...
	function resetPasswdClick(){
		...
		var form_equalTopwd = $('#form-equalTopwd').val();
        // if (! imgCode) {
        //     return Public.tips.alert('请输入图形验证码');
        // }

        if (!mobile) {
             return Public.tips.alert('手机号不能为空');
        }

        if (/[ !@#$%^&*()_+<>]+/.test(user_password)){
            return Public.tips.alert('密码不能存在特殊符号');
        }
        ...
        if (user_password != form_equalTopwd) {
        	return Public.tips.alert('新密码和确认密码不一致');
        }
