        ...
        <div class="reg-form fl">
            <form action="" id="register-form1" method="post" novalidate="novalidate"  onsubmit="return false;">
                <div id="form-item-password" class="form-item" style="z-index: 12;">
                    <label>BP编号：</label>
                    <input type="text" id="par_member" class="field re_user_password" autocomplete="off" maxlength="20" placeholder="BP编号" default="<i class=i-def></i><?=$pwd_str?>" onchange="instruClick()">
                </div>
                <div class="input-tip">
                    <span></span>
                </div>
                <div id="form-item-password" class="form-item" style="z-index: 12;">
                    <label>BP姓名：</label>
                    <input type="text" id="par_name" class="field re_user_password" autocomplete="off" maxlength="20" placeholder="BP姓名" default="<i class=i-def></i><?=$pwd_str?>">
                </div>
                <div class="input-tip">
                    <span></span>
                </div>
            ...
            <form action="" id="register-form4" method="post" novalidate="novalidate"  onsubmit="return false;">
                <div style="text-align: center">
                    ...
                    <br>
                        ...
                        您的BP编号是：<p id="parent_member">xxxxxx</p> 
                    </div>
...
<script>
        ...
        $.ajax({ 
              ...
              success:function (res){
                if(res.status == 250)
                {
                    ...
                    alert("成功");
                    setCookie("cardinfo",JSON.stringify(res.data),7);  //序列化 反序列化JSON.parse()
                    console.log(getCookie("cardinfo"));
                    // window.location.href = '<?=sprintf('%s?ctl=Login&act=regidimg1&t=%s&from=%s&callback=%s&c=%s', Yf_Registry::get('url'), request_string('t'), request_string('from'), urlencode(request_string('callback')),request_string('t'))?>'+$.base64.encode(JSON.stringify(res.data));
                }else{ 
    ...
    if (cardinfo) {
        ...
        // $("#par_member").attr(cardinfo[0]);
        // $("#par_name").attr(cardinfo[1]);
        ...
        $.ajax({
            ...
            success: function (respone) {
                if (respone.status == 200) {
                    ...
                    setCookie("BelongSalon",respone.data['BelongSalon'],7); //归属沙龙编号
                    setCookie("ParentMemberName",respone.data['ParentMemberName'],7);  //辅导员姓名
        ...
        $.ajax({
            ...
            success: function (respone) {
                if (respone.status == 250) {
                    ...
                    window.countDown();
                    setCookie("phone_code",respone.data['user_code'],7);
    ...
    function reg1Click(){
        ...
        var regstep     = $('#regstep1').val();
        var par_member  = $('#par_member').val();
        ...
        var phone_code  = getCookie("phone_code"); //获取验证码

        var BelongSalon = getCookie("BelongSalon"); //归属沙龙

        if (!par_member) {
            return Public.tips.alert('请输入BP编号');
        }
        if (!par_name) {
            return Public.tips.alert('请输入BP姓名');
        }
        if (!user_idcard) {
        ...
        var ajaxurl = './index.php?ctl=Login&met=newregister&typ=json';
        $.ajax({
            type: "POST",
            url: ajaxurl,
            dataType: "json",
            ...
            success: function (respone)
            {
                if(respone.status == 200){
                    alert("成功");
                    setCookie("user_name",respone.data['user_name'],7);
                    setCookie("user_id",respone.data['user_id'],7);
                    console.log(respone.data['user_name'],respone.data['user_id']);
                    //Public.tips.alert('');
                }
            }
        });
        ...
        $.ajax({ 
              ...
              success:function (res){
                 if(res.status == 200){
                    ...
                    setCookie("bankinfo",JSON.stringify(res.data),7);  //序列化 反序列化JSON.parse()
    ...
    function reg3Click(){
        ...
        var user_id          = getCookie("user_id");
        var user_name        = getCookie("user_name");
        var regstep          = $('#regstep3').val();
        var re_user_password = $('#re_user_password').val(); //新密码
        var password         = $('#form_equalTopwd').val();  //确认密码

        var param = {
            "user_id": user_id,  //主键
            "user_name": user_name,  //主键
            "regstep": regstep,
            "user_password": password    //传输确认密码
        };
        ...
        $.ajax({
            ...
            success: function (respone)
            {
                if(respone.status == 200){
                    alert("成功");
                    setCookie("member_id",respone.data['MemberID'],7);
                    setCookie("par_member",respone.data['par_member'],7);

                    $("#member_id").text(getCookie("member_id"));
                    $("#parent_member").text(getCookie("par_member"));

                    console.log(respone.data['MemberID'],respone.data['par_member']);

                    //跳转下一页

                }else{
                    Public.tips.alert('注册失败，'+respone.meg);
                    //三秒后跳转首页
                    return;
                }
            }
