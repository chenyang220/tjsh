   ...
   (function ($) {
    $.fn.extend({
        "citylist": function (params) {
            params = $.extend({
                id: 'id',                   //鏁版嵁鐨処D鍚嶇О
                name: 'name',                //鏁版嵁鐨刅ALUE鍚嶇О
                children: 'children',        //鏁版嵁瀛愬厓绱犵殑KEY鐨勫悕绉�
                metaTag: 'data-extra',       //灏嗘暟鎹厓绱犵殑ID瀛樻斁鍒癏TML鍏冪礌鐨勬爣绛句腑
                idVal: false,               //浣跨敤ID鍋歄PTION鐨刅ALUE
                data: false,                 //榛樿鏁版嵁
                selected: false              //閫変腑鐨勫厓绱爀g: 14 / [14:1401]
            }, params);
...
$(function () {
    ...
    {id: 1101, name: '北京'}
                ...
                function defaultSite(p,c){
                  
                    $("#province option:contains("+p+")").attr("selected","selected");
                    var Pe = $("#province").find("option:selected").attr("name");
                    console.log(Pe);
                    $('#province, #city').citylist({data:data, id:'id',children:'citys',name:'name',metaTag:'name',selected:[Pe,]});
                    $("#city option:contains("+c+")").attr("selected","selected");
                    var Cy = $("#city").find("option:selected").attr("name");
                    console.log(Cy);
                    $('#province, #city').citylist({data:data, id:'id', children:'citys',name:'name',metaTag:'name',selected:[Pe,Cy]});
                }


                var address = JSON.parse(sessionStorage.getItem('site'));
                var province = address['province'];
                var city = address['city'];
                sessionStorage.removeItem('site');
                defaultSite(province,city);
