...
var ApiUrl = 'http://139.196.51.206/bbc_tjsh/shop/index.php';
...
UploadImage = function (uploadArguments) {
    ...
    if (typeof(uploadConfig) === 'undefined'){
        ...
        $.ajax({
            type: "get",
            url: ApiUrl + "?ctl=Upload&action=config&typ=json",
            dataType: "jsonp",
            jsonp: "callback",//传递给请求处理程序或页面的，用以获得jsonp回调函数名的参数名(一般默认为:callback)
            // jsonpCallback:"getConfig",//自定义的jsonp回调函数名称，默认为jQuery自动生成的随机函数名，也可以写"?"，jQuery会自动为你处理数据
            success: function(data){
...
//                },
                ...
                swf: ApiUrl + '/shop_admin/static/common/js/Uploader.swf',

                server: ApiUrl + "?ctl=Upload&action=" + $action+'&typ=json',
        ...
        uploader.on('uploadSuccess', function (file, response) {
            ...
            if (response.state == 'SUCCESS') {
                ...
                $.ajax({
                    type: "get",
                    url: ApiUrl + "/index.php?ctl=User_User&met=editUserLogo&typ=json",
                    data: {k:getCookie('key'),u:getCookie('id'),img:response.url},
                    dataType: "json",
                    success: function (e){
                        if(e.status==200){
                            $('#photo_goods_logo').attr('src', response.url);
                            //$img.attr('_src',response.url);
